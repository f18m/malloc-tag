...
typedef struct MallocTreeNode_s
{
    ...
    void set_sitename_to_shlib_name_from_func_pointer(void *funcpointer)
    {
        ...
        if (dladdr(funcpointer, &address_info) == 0 || address_info.dli_fname == nullptr)
        {
            ...
            strncpy(&m_siteName[0], address_info.dli_fname, std::min(strlen(address_info.dli_fname), (size_t)MAX_SITENAME_LEN));
...
typedef struct MallocTree_s
{
    ...
    bool is_ready()
    {
        return m_node_pool != NULL && m_root_node != NULL && m_current_node != NULL;
    }
...
void *
mtag_malloc_hook(size_t size, void *caller)
{
